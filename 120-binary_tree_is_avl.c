...
int btia(const binary_tree_t *tree, int min, int max);
/**
...
int binary_tree_is_avl(const binary_tree_t *tree)
{
	...
	return (btia(tree, INT_MIN, INT_MAX));
}

/**
 * btia - helper that finds if a binary tree is an avl
 * @tree: pointer to the root node of the tree
 * @min: minimum value
 * @max: maximum value
 * Return: 1 if tree is avl
...
int btia(const binary_tree_t *tree, int min, int max)
{
	...
	return (btia(tree->left, min, tree->n - 1) &&
		btia(tree->right, tree->n + 1, max));
}
